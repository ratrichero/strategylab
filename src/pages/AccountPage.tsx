...
export function AccountPage() {
  ...
  const fetchTradeHistory = async () => {
    ...
    if (!raw) { toast.error("Please enter symbol, e.g. BTC or BTCUSDT"); return; }
    const symbol = raw.endsWith("USDT") ? raw : `${raw}USDT`;
    try {
      setTradesLoading(true);
      const res = await fetch(`${API}/account/trades?target=${currentTarget}&symbol=${symbol}&limit=500`);
      const data = await res.json();
      if (!res.ok) throw new Error(data?.detail || "Failed to fetch trades");
  ...
  const kpi = useMemo(() => {
    ...
    return { totalWalletBalance, totalUnrealizedProfit, totalMarginBalance, availableBalance, totalPositionInitialMargin, totalOpenOrderInitialMargin, marginLevel: totalPositionInitialMargin > 0 ? totalMarginBalance / totalPositionInitialMargin : 0 };
  }, [accountInfo]);

  const activePositions = useMemo(() => positions.filter(p => Math.abs(Number(p.positionAmt || 0)) > 0), [positions]);
  const posDistribution = useMemo(() => activePositions.map(p => ({ name: p.symbol, value: Math.abs(Number(p.notional || 0)), pnl: Number(p.unrealizedProfit || 0) })), [activePositions]);
  ...
  const filteredTrades = useMemo(() => {
    return tradeHistory.filter(t => {
      ...
      if (hStartDate || hEndDate) {
        const ts = Number(t.time || 0); if (!ts) return false;
        const VN_MS = 7 * 3600 * 1000; const vn = new Date(ts + VN_MS);
  ...
  const tradeStats = useMemo(() => {
    ...
    return { total: filteredTrades.length, volume: filteredTrades.reduce((s, t) => s + Math.abs(Number(t.quoteQty || 0)), 0), commission: filteredTrades.reduce((s, t) => s + Math.abs(Number(t.commission || 0)), 0), pnl: filteredTrades.reduce((s, t) => s + Number(t.realizedPnl || 0), 0) };
  }, [filteredTrades]);
  const incomeStats = useMemo(() => {
    const byType = {}; filteredIncome.forEach(i => { const t = i.incomeType || "UNKNOWN"; if (!byType[t]) byType[t] = 0; byType[t] += Number(i.income || 0); });
  ...
  const posColumns = [
    ...
    { key: "positionAmt", header: "Size", sortable: true, align: "right", render: v => <span className={Number(v) > 0 ? "text-emerald-400" : "text-red-400"}>{Number(v).toFixed(4)}</span> },
      ...
      <TabContent>
        {tab === "overview" && (<div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card><CardHeader title="Balance Breakdown" /><div className="space-y-3">{[["Wallet Balance", kpi?.totalWalletBalance, "text-white"], ["Unrealized PnL", kpi?.totalUnrealizedProfit, pnlClr(kpi?.totalUnrealizedProfit)], ["Margin Balance", kpi?.totalMarginBalance, "text-white"], ["Available Balance", kpi?.availableBalance, "text-emerald-400"], ["Position Margin", kpi?.totalPositionInitialMargin, "text-yellow-400"], ["Order Margin", kpi?.totalOpenOrderInitialMargin, "text-slate-400"]].map(([label, val, clr]) => (<div key={label} className="flex justify-between py-2 border-b border-slate-700 last:border-0"><span className="text-slate-400">{label}</span><span className={`font-bold font-mono ${clr}`}>${$n(val)}</span></div>))}</div></Card>
