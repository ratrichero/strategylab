...
export function Indicators() {
  ...
  const kpi = (()=>{const t=filtered.length,w=wins.length;if(!t)return{total:0,wr:0,pf:0};const wr=(w/t)*100;const gp=filtered.filter(s=>(s.result_percent||0)>0).reduce((a,s)=>a+(s.result_percent||0),0);const gl=Math.abs(filtered.filter(s=>(s.result_percent||0)<0).reduce((a,s)=>a+(s.result_percent||0),0));const pf=gl>0?gp/gl:gp>0?Infinity:0;return{total:t,wr,pf};})();
    ...
    <div className="space-y-6">
      ...
      <TabContent>
        ...
        {tab==='heatmap'&&<div className="space-y-6"><Card><CardHeader title="RSI x Volume Ratio -> Win Rate" />{ad.rsi_vol_heatmap?.length?<Heatmap data={ad.rsi_vol_heatmap.map(r=>({x:r.vol_zone,y:r.rsi_zone,value:Number(r.win_rate)||50,count:Number(r.n_trades)||0}))} xLabel="Volume" yLabel="RSI" valueLabel="WR%" colorScale="green-red" showValues />:<div className="h-48 flex items-center justify-center text-slate-500">No data</div>}</Card><Card><CardHeader title="ATR x Score -> Win Rate" />{ad.atr_score_heatmap?.length?<Heatmap data={ad.atr_score_heatmap.map(r=>({x:r.score_band,y:r.atr_bucket,value:Number(r.win_rate)||50,count:Number(r.n_trades)||0}))} xLabel="Score" yLabel="ATR%" valueLabel="WR%" colorScale="green-red" showValues />:<div className="h-48 flex items-center justify-center text-slate-500">No data</div>}</Card><Card><CardHeader title="MTF x Trend -> Win Rate" />{ad.mtf_trend_heatmap?.length?<Heatmap data={ad.mtf_trend_heatmap.map(r=>({x:r.trend_bucket,y:r.mtf_bucket,value:Number(r.win_rate)||50,count:Number(r.n_trades)||0}))} xLabel="Trend" yLabel="MTF" valueLabel="WR%" colorScale="green-red" showValues />:<div className="h-48 flex items-center justify-center text-slate-500">No data</div>}</Card>
            <Card><div className="flex items-center gap-4 mb-4"><CardHeader title="Indicator x Indicator Scatter" subtitle="Find sweet spot zones" /><select value={scatterX} onChange={e=>setScatterX(e.target.value)} className="bg-slate-800 border border-slate-600 rounded-lg px-3 py-1.5 text-white text-sm"><option value="rsi">RSI</option><option value="volume_ratio">Vol Ratio</option><option value="atr_ratio">ATR Ratio</option><option value="score">Score</option></select><span className="text-slate-500">x</span><select value={scatterY} onChange={e=>setScatterY(e.target.value)} className="bg-slate-800 border border-slate-600 rounded-lg px-3 py-1.5 text-white text-sm"><option value="volume_ratio">Vol Ratio</option><option value="rsi">RSI</option><option value="atr_ratio">ATR Ratio</option><option value="score">Score</option></select></div>{indScatterData.length>0?<ResponsiveContainer width="100%" height={350}><ScatterChart><CartesianGrid strokeDasharray="3 3" stroke="#334155" /><XAxis type="number" dataKey="x" name={INDICATOR_LIST.find(i=>i.value===scatterX)?.label} stroke="#64748b" fontSize={11} /><YAxis type="number" dataKey="y" name={INDICATOR_LIST.find(i=>i.value===scatterY)?.label} stroke="#64748b" fontSize={11} /><Tooltip content={<ChartTT />} /><Scatter data={indScatterData.filter(d=>d.label===1)} fill="#10b981" name="Win" /><Scatter data={indScatterData.filter(d=>d.label===0)} fill="#ef4444" name="Loss" /></ScatterChart></ResponsiveContainer>:<div className="h-48 flex items-center justify-center text-slate-500">No data</div>}</Card>
          </div>}
