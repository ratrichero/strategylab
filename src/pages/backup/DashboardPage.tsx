...
import { parseUtcMs, utcToVN, getTodayVN, toUtcRangeFromVNDate, normalizeSignalDates } from '../utils/time';
...
export function Dashboard() {
  ...
  const [metricSignals, setMetricSignals] = useState([]);
    ...
    (async () => {
      ...
      try {
        ...
        const [allSigs, metricSigs, pending, vers, prices, appCfg] = await Promise.all([
          ...
          fetchAPI(`/signals?limit=2000&date_field=exit_time&start_date=${encodeURIComponent(start)}&end_date=${encodeURIComponent(end)}`).catch(() => ({ data: [] })),
        ...
        const metrics = (metricSigs.data || []).map(normalizeSignalDates);
        console.log('[Dashboard] all:', signals.length, '| metric:', metrics.length);

        setAllSignals(signals);
        setMetricSignals(metrics);
  ...
  // Refetch metric dataset when applied date changes
  useEffect(() => {
    (async () => {
      try {
        ...
        const sigs = await fetchAPI(`/signals?limit=2000&date_field=exit_time&start_date=${encodeURIComponent(start)}&end_date=${encodeURIComponent(end)}`);
        const metrics = (sigs.data || []).map(normalizeSignalDates);
        console.log('[Dashboard] Refetch result:', metrics.length);
        setMetricSignals(metrics);
  ...
  const metricClosed = useMemo(() => metricSignals.filter(s => s.status === 'WIN' || s.status === 'LOSS').sort((a, b) => parseUtcMs(b.exit_time || '') - parseUtcMs(a.exit_time || '')), [metricSignals]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <Card className="flex items-center gap-4"><div className="p-3 bg-indigo-500/20 rounded-xl"><BarChart3 className="w-6 h-6 text-indigo-400" /></div><div><p className="text-sm text-slate-400">Total Trades</p><p className="text-2xl font-bold text-white">{totalTrades}</p></div></Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            }} className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg text-xs">📋 Trade Streak</button>
          </div>
          <div className="space-y-3">
            ...
            <div className="flex justify-between py-2 border-b border-slate-700"><span className="text-slate-400">Max Loss Streak <span className="text-slate-600 text-xs">(Candle / Exit)</span></span><span className="font-bold text-red-400">{streaks.candle.maxLoss} / {streaks.exit.maxLoss}</span></div>
            <div className="flex justify-between py-2 border-b border-slate-700"><span className="text-slate-400">Max Win Streak <span className="text-slate-600 text-xs">(Candle / Exit)</span></span><span className="font-bold text-emerald-400">{streaks.candle.maxWin} / {streaks.exit.maxWin}</span></div>
            <div className="flex justify-between py-2 border-b border-slate-700"><span className="text-slate-400">Long Win Rate</span><span className={`font-bold ${longShortWR.longWR >= 50 ? 'text-emerald-400' : 'text-red-400'}`}>{longShortWR.longWR.toFixed(1)}% <span className="text-slate-500 text-xs">({longShortWR.longTotal})</span></span></div>
            <div className="flex justify-between py-2 border-b border-slate-700"><span className="text-slate-400">Short Win Rate</span><span className={`font-bold ${longShortWR.shortWR >= 50 ? 'text-emerald-400' : 'text-red-400'}`}>{longShortWR.shortWR.toFixed(1)}% <span className="text-slate-500 text-xs">({longShortWR.shortTotal})</span></span></div>
      ...
      {/* RECENT (UNFILTERED) */}
      <Card>
        <CardHeader title="Recent Trades" subtitle={`${filteredClosed.length} closed today • By exit time`} />
