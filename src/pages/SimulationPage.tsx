...
export function SimulationPage() {
  ...
  const loadResults = async (id) => {
    try {
      const sumResp = await fetch(`${API}/backtest/replay/jobs/${id}/summary`);
      const sumRes = await sumResp.json();

      if (!sumResp.ok) {
        const msg = typeof sumRes.detail === 'string'
          ? sumRes.detail
          : JSON.stringify(sumRes.detail || sumRes.error || 'Summary failed');
        throw new Error(msg);
      }
      ...
      if (!rowsResp.ok) {
        const msg = typeof rowsRes.detail === 'string'
          ? rowsRes.detail
          : JSON.stringify(rowsRes.detail || rowsRes.error || 'Rows failed');
        throw new Error(msg);
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3"><FlaskConical className="w-7 h-7 text-purple-400" /><div><h2 className="text-2xl font-bold text-white">Signal Replay Backtest</h2><p className="text-slate-400 mt-0.5">Replay tín hiệu thật theo policy % — so sánh Actual vs Simulated</p></div></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card>
            ...
            <div className="grid grid-cols-12 gap-4 pt-3 border-t border-slate-700">
              <div className="col-span-2"><p className="text-[10px] text-slate-500 uppercase font-semibold mb-1.5">TF</p><div className="flex gap-1">{TIMEFRAMES.map(tf => <button key={tf} onClick={() => toggleArr(setTimeframes, tf)} className={`px-2.5 py-1.5 rounded text-xs ${timeframes.includes(tf) ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-slate-400'}`}>{tf}</button>)}</div></div>
            ...
            <p className="text-[10px] text-slate-600 mt-2">💡 Symbols: nhập tên viết tắt, cách nhau bằng dấu cách hoặc dấu phẩy.</p>
          </Card>
        </div>
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            {uniformMode ? <div className="text-[10px] text-indigo-400 bg-indigo-500/10 px-2 py-1.5 rounded">ℹ️ Uniform: cùng 1 bộ SL/TP/Levels áp cho tất cả timeframe.</div> : (<>
              <div className="text-[10px] text-amber-400 bg-amber-500/10 px-2 py-1.5 rounded">⚠️ Per Timeframe: chỉnh riêng cho <b>{activeTf}</b>.</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs text-slate-400 mb-1">SL (%)</label><input type="number" step="0.1" value={currentTfPolicy.sl_pct || ''} onChange={e => updateTfField(activeTf, 'sl_pct', e.target.value)} className="w-full px-2 py-1.5 bg-slate-900 border border-slate-600 rounded text-sm text-white font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="2.0" /></div>
              <div><label className="block text-xs text-slate-400 mb-1">TP (%)</label><input type="number" step="0.1" value={currentTfPolicy.tp_pct || ''} onChange={e => updateTfField(activeTf, 'tp_pct', e.target.value)} className="w-full px-2 py-1.5 bg-slate-900 border border-slate-600 rounded text-sm text-white font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="4.0" /></div>
            </div>
            <div>
              ...
              {(currentTfPolicy.levels || []).map((lv, i) => (
                <div key={i} className="p-3 bg-slate-900/50 rounded-lg border border-slate-700/50 mb-2">
                  <div className="flex items-center justify-between mb-2"><span className="text-xs text-slate-300 font-medium">Level {i + 1}</span><button onClick={() => removeLevel(activeTf, i)} className="text-xs text-red-400 hover:text-red-300">Remove</button></div>
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <div><label className="block text-[10px] text-slate-500 mb-0.5">Trigger (%)</label><input type="number" step="0.1" value={lv.trigger_pct || ''} onChange={e => updateLevel(activeTf, i, 'trigger_pct', e.target.value)} className="w-full px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs text-white font-mono" placeholder="2.0" /></div>
                    <div><label className="block text-[10px] text-slate-500 mb-0.5">Action</label><select value={lv.action} onChange={e => updateLevel(activeTf, i, 'action', e.target.value)} className="w-full px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs text-white">{ACTIONS.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}</select></div>
                  </div>
                  {lv.action === 'move_to_entry' && <div><label className="block text-[10px] text-slate-500 mb-0.5">Buffer (%)</label><input type="number" step="0.01" value={lv.buffer_pct || ''} onChange={e => updateLevel(activeTf, i, 'buffer_pct', e.target.value)} className="w-full px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs text-white font-mono" placeholder="0.2" /></div>}
                  {lv.action === 'move_stop_to_profit_pct' && <div><label className="block text-[10px] text-slate-500 mb-0.5">Target Profit (%)</label><input type="number" step="0.1" value={lv.target_profit_pct || ''} onChange={e => updateLevel(activeTf, i, 'target_profit_pct', e.target.value)} className="w-full px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs text-white font-mono" placeholder="1.5" /></div>}
                </div>
              ))}
            ...
            <div className="text-[10px] text-slate-600">Tất cả giá trị nhập theo %. Intrabar = Conservative | Horizon: 15m=24h, 1h=72h, 4h=7d</div>
      ...
      {/* Empty state */}
      {!jobId && !summary && (<Card className="flex items-center justify-center h-64"><div className="text-center"><FlaskConical className="w-12 h-12 text-slate-700 mx-auto mb-3" /><p className="text-slate-500">Chưa có kết quả backtest</p><p className="text-xs text-slate-600 mt-1">Cấu hình bộ lọc và policy, nhấn Run Backtest</p></div></Card>)}
      ...
      {selectedTrade && tradeDetail && (<div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => { setSelectedTrade(null); setTradeDetail(null); }}><div className="bg-slate-800 border border-slate-700 rounded-xl p-6 max-w-3xl w-full max-h-[85vh] overflow-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {tradeDetail.policy_levels?.length > 0 && <div className="mt-6 p-4 bg-slate-900/50 rounded-lg"><h4 className="text-sm font-semibold text-slate-300 mb-3">Policy Levels</h4><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{tradeDetail.policy_levels.map((lv, i) => <div key={i} className="bg-slate-800/50 rounded-lg p-3 text-center"><p className="text-xs text-slate-400">{lv.name} ({lv.trigger_r}R)</p><p className="text-sm text-white font-mono mt-1">→ {fmtPrice(lv.trigger_price)}</p><p className="text-xs text-yellow-400 mt-0.5">Stop: {fmtPrice(lv.stop_after_trigger)}</p></div>)}</div></div>}
