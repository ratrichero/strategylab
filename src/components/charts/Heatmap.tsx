...
interface HeatmapProps {
  ...
  yLabel?: string;
  valueLabel?: string;
  colorScale?: 'green-red' | 'blue-orange' | 'single';
  showValues?: boolean;
...
export function Heatmap({
  ...
  const getColor = (value: number): string => {
    ...
    } else if (colorScale === 'blue-orange') {
      if (normalized >= 0.5) {
        const intensity = (normalized - 0.5) * 2;
  ...
  const getTextColor = (value: number): string => {
    const normalized = (value - minValue) / (maxValue - minValue || 1);
    return normalized > 0.3 && normalized < 0.7 ? '#cbd5e1' : '#ffffff';
  };
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <div className="flex items-center justify-end gap-4 text-sm">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-slate-400">{minValue.toFixed(1)}</span>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-slate-400">{maxValue.toFixed(1)}</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <div className="inline-block min-w-full">
          {/* X-axis labels */}
          <div className="flex">
            <div className="w-40 flex-shrink-0" /> {/* Spacer for Y labels */}
            {xValues.map(x => (
              <div
                key={x}
                className="flex-1 min-w-24 px-1 py-2 text-center text-sm text-slate-300 font-medium"
              >
                {x}
              </div>
            ))}
          ...
          {yValues.map(y => (
            <div key={y} className="flex">
              {/* Y-axis label — full width, no truncate */}
              <div className="w-40 flex-shrink-0 px-2 py-2 text-right text-sm text-slate-300 font-medium">
                {y}
              </div>
              {/* Cells */}
              {xValues.map(x => {
                ...
                return (
                  <div
                    key={`${x}-${y}`}
                    className={cn(
                      'flex-1 min-w-24 min-h-12 flex items-center justify-center',
                      'm-0.5 rounded transition-transform',
                      onCellClick && 'cursor-pointer hover:scale-105'
                    )}
                    style={{ backgroundColor: getColor(value) }}
                    onClick={() => cell && onCellClick?.(cell)}
                    title={cell ? `${xLabel || 'X'}: ${x}, ${yLabel || 'Y'}: ${y}, ${valueLabel}: ${value.toFixed(2)}${cell.count ? `, Count: ${cell.count}` : ''}` : ''}
                  >
                    {showValues && cell && (
                      <span
                      ...
                      >
                        {value.toFixed(1)}%{cell.count !== undefined ? ` · ${cell.count}` : ''}
                      </span>
                    )}
                  </div>
                );
