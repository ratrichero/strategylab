...
import {
  LayoutDashboard, TrendingUp, Zap, BarChart3, Cpu,
  Ban, FlaskConical, Code2, Settings, Microscope,
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const pageTitle = [...nav, { href: "/settings", name: "Settings" }]
    .find(n => n.href === location.pathname || (n.href !== "/" && location.pathname.startsWith(n.href)))
    ?.name || "Dashboard";
      ...
      <aside className={cn(
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-700/50 flex-shrink-0">
          {!sidebarCollapsed && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-white">Quant Lab</span>
            </div>
          )}
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          {nav.map((item) => {
            ...
            return (
              <Link key={item.href} to={item.href} title={sidebarCollapsed ? item.name : undefined}
                className={cn(
        ...
        <div className="p-2 border-t border-slate-700/50 flex-shrink-0">
          <Link to="/settings" title={sidebarCollapsed ? "Settings" : undefined}
      ...
      <div className={cn("flex-1 flex flex-col min-h-screen transition-all duration-300", sidebarCollapsed ? "ml-16" : "ml-64")}>
        <header className="h-16 flex-shrink-0 sticky top-0 z-40 flex items-center justify-between px-6 bg-slate-800/30 backdrop-blur-sm border-b border-slate-700/50">
          ...
          <div className="flex items-center gap-3">
            <StatusBar />
            <KillSwitch />
            <button onClick={() => setSearchOpen(true)} className="p-2 rounded-lg hover:bg-slate-700/50 text-slate-400 hover:text-white transition-colors">
        ...
        {searchOpen && (
          <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-start justify-center pt-24 px-4" onClick={() => setSearchOpen(false)}>
            <div className="w-full max-w-2xl bg-slate-800 border border-slate-700 rounded-xl shadow-2xl p-4" onClick={(e) => e.stopPropagation()}>
              <input autoFocus type="text" placeholder="Search signals, patterns, queries..."
