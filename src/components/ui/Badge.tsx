...
export function Badge({ children, variant = "default", size = "md", className }: BadgeProps) {
  ...
  const s = { sm: "px-1.5 py-0.5 text-xs", md: "px-2 py-1 text-xs" };
  return (
    <span className={cn("inline-flex items-center font-medium rounded-md", v[variant], s[size], className)}>
      {children}
    </span>
  );
}
...
export function StatusBadge({ status, className }: { status: string; className?: string }) {
  const map: Record<string, { variant: BadgeProps["variant"]; label: string }> = {
    ...
    LONG: { variant: "success", label: "LONG" },
    SHORT: { variant: "danger", label: "SHORT" },
    BULL: { variant: "success", label: "BULL" },
    BEAR: { variant: "danger", label: "BEAR" },
    SIDEWAYS: { variant: "warning", label: "SIDEWAYS" },
    RANGING: { variant: "warning", label: "RANGING" },
    VOLATILE: { variant: "info", label: "VOLATILE" },
  };
...
export function DirectionBadge({ direction, className }: { direction: string; className?: string }) {
  return (
    <Badge variant={direction === "LONG" ? "success" : "danger"} className={cn("uppercase", className)}>
...
export function ScoreBadge({ value, className }: { value: number; className?: string }) {
  ...
  return (
    <span className={cn("font-mono text-sm", v >= 8 ? "text-emerald-400" : v >= 6 ? "text-yellow-400" : "text-red-400", className)}>
      {v.toFixed(2)}
    </span>
  );
}
