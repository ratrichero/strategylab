"""Dashboard Research API — PATCHED: unified VN date parsing"""
...
async def run_research(config: ResearchConfig):
    try:
        ...
        start_dt = _parse_vn_date(config.start_date, is_end=False)
        end_dt = _parse_vn_date(config.end_date, is_end=True)

        if start_dt:
            conds.append(f"{date_col} >= ${idx}"); params.append(start_dt); idx += 1
        if end_dt:
            conds.append(f"{date_col} < ${idx}"); params.append(end_dt); idx += 1
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(f"""
                SELECT s.*,
                    sf.rsi AS sf_rsi, sf.volume_ratio AS sf_vol,
                    sf.atr_ratio AS sf_atr, sf.ema_distance AS sf_ema,
        ...
        for r in rows:
            ...
            # Symbol filter (client-side for flexibility)
            if config.symbols:
                sym_list = [x.strip().upper() + ("" if x.strip().upper().endswith("USDT") else "USDT")
                           for x in config.symbols.replace(",", " ").split() if x.strip()]
                if sym_list:
            ...
            if entry > 0 and (has_sl_tp or has_rr):
                if has_sl_tp:
                    sl_pct = abs(config.sl_pct)
                    tp_pct = abs(config.tp_pct)
                else:
                ...
                if hit_sl and hit_tp:
                    if exit_reason == "sl":
                        sim_result = -sl_pct; sim_status = "LOSS"
                    elif exit_reason == "tp":
                        sim_result = tp_pct; sim_status = "WIN"
                    else:
                        sim_result = None; sim_status = "NOT_COUNT"; sim_counted = False
                elif hit_sl:
                    sim_result = -sl_pct; sim_status = "LOSS"
                elif hit_tp:
                    sim_result = tp_pct; sim_status = "WIN"
                else:
                    sim_result = result; sim_status = d.get("status", "")
            ...
            clean.update({
                ...
                "_debug_mae": mae,
                "_debug_mfe": mfe,
                "_debug_sl_pct": sl_used,
                "_debug_tp_pct": tp_used,
                "_debug_exit_reason": exit_reason,
                "_debug_original_result": result,
                "_debug_hit_sl": sl_used is not None and mae >= sl_used,
                "_debug_hit_tp": tp_used is not None and mfe >= tp_used,
                "rsi_val": rsi,
                "volume_ratio_val": vr,
            })
            trades.append(clean)
