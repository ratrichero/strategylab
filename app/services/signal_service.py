...
def get_effective_threshold(strategy_name, pattern_name, runtime_cfg):
    """
    Lấy threshold theo thứ tự ưu tiên:
    1. STRATEGY_CONFIG[strategy].patterns[pattern]
    2. STRATEGY_CONFIG[strategy].threshold
    3. STRATEGY_THRESHOLDS[strategy] (Legacy fallback)
    4. SCORE_THRESHOLD global
    """
    global_threshold = float(runtime_cfg.get("SCORE_THRESHOLD", 5.0))

    # 1. Thử lấy từ STRATEGY_CONFIG (Cấu trúc mới: lồng ghép)
    strategy_cfg_all = runtime_cfg.get("STRATEGY_CONFIG", {})
    strategy_cfg = strategy_cfg_all.get(strategy_name, {})

    if isinstance(strategy_cfg, dict):
        # Ưu tiên Pattern-specific threshold
        pattern_cfg = strategy_cfg.get("patterns", {})
        if isinstance(pattern_cfg, dict):
            pattern_threshold = pattern_cfg.get(pattern_name)
            if pattern_threshold is not None:
                return float(pattern_threshold)

        # Kế đến là Strategy default threshold
        strategy_default = strategy_cfg.get("threshold")
        if strategy_default is not None:
            return float(strategy_default)

    # 2. Thử lấy từ STRATEGY_THRESHOLDS (Legacy fallback: phẳng)
    flat_thresholds = runtime_cfg.get("STRATEGY_THRESHOLDS", {})
    flat_val = flat_thresholds.get(strategy_name)
    if flat_val is not None:
        return float(flat_val)

    # 3. Cuối cùng dùng Global Score Threshold
