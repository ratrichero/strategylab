...
def _metrics(trades):
    ...
    wins    = int((returns > 0).sum()); losses = len(returns) - wins
...
def _generate(report_type, days, title):
    ...
    m_p = _metrics(period_trades); m_a = _metrics(all_trades)
    ...
    if strat_lines: report += f"\n<b>═══ STRATEGY ═══</b>\n{strat_lines}"
    if best: report += f"\n<b>🏆 Best:</b> {best.symbol} {best.direction} {float(best.result_percent):+.2f}%\n"
    if worst: report += f"<b>💀 Worst:</b> {worst.symbol} {worst.direction} {float(worst.result_percent):+.2f}%\n"
    report += (f"\n<b>═══ STATUS ═══</b>\n"
    ...
    try:
        ...
        db2 = SessionLocal()
        db2.execute(text("INSERT INTO reports (report_type,period_start,period_end,content,created_at) "
...
def send_daily():   report = _generate("daily",   1,  "📊 DAILY REPORT");   send_telegram(report); return report
def send_weekly():  report = _generate("weekly",  7,  "📊 WEEKLY REPORT");  send_telegram(report); return report
def send_monthly(): report = _generate("monthly", 30, "📊 MONTHLY REPORT"); send_telegram(report); return report
